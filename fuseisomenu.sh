...
FUSEPREFIX=""  # mountdir prefix when not asking for specific mountdir
...
if [ -n "$mountbasedir" ]; then
    ...
    #if [ $QUIET_KDIALOG -ne 1 ]; then
        kdialog --title "$TITLE" --warningyesno "Are you sure you want to unmount all the following ISOs?
            $(awk '{printf "%s -> %s\n", $1, $2}' $MTAB_FUSEISO)"
    #fi
    if [ $? -eq 0 ]; then
